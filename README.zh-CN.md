...
- `浏览器辅助抓取`：任务页只保存播放器实际请求过的 HLS 或 DASH 分片，默认单队列且不预抓；保持任务页开启最可靠。短暂关闭时 background 会暂存近期请求元数据，重开后尽力重试，但不会缓存浏览器已经收到的响应体；
- `本次忽略`：不处理当前视频。

下载中心里不想要的发现可以直接清掉：每条还没创建下载的发现都有「移除这个发现」，顶部还有一个按钮一次清空全部。这只删掉发现记录，不影响已创建的下载、已保存文件和断点；重新播放那个视频还会再次发现它。
...
Capture 会跟随播放器实际使用的清晰度：在还没有保存任何内容之前，会自动切换到包含这些分片的那个清晰度列表；保存第一个分片之后清晰度就固定下来，之后播放器再换清晰度只会提示，不会把两种分辨率混进同一个文件。暂时对不上播放列表的分片会先记下来，等播放列表更新后重试。任务页自己请求不到的分片，会改在原网页会话里请求，从而带上该页面的 Cookie、Referer 和 Origin。

DASH 的辅助抓取按同样的方式工作：先从 MPD 建立所有 Representation 的索引，播放器第一次请求的视频轨和音频轨会被固定为本任务的轨道，两条轨道分别断点保存，最后在浏览器内合并成一个 CMAF MP4。选定的轨道 id 保存在任务里，重开任务仍是同两条轨道。

辅助抓取启动后，Web Keeper 会在视频页面内保留播放器已经收到的媒体数据（上限约 96 MB），任务页优先直接取这份字节，取不到才回落到重新请求。所以一次性 URL、绑定播放会话的鉴权通常也能保存。注意它只对注入之后播放的内容有效：之前已经播过的部分需要重新播一次。任务完成或移除任务时会还原页面并清空这份缓冲。

直播（HLS 直播列表或 `type="dynamic"` 的 MPD）不会自动生成视频，会持续保存到你点「检查并生成视频」为止。

辅助抓取任务页可以加速。「播放加速」先尝试网页播放器自己的 `playbackRate`（2x / 4x / 8x，自动静音）；站点禁用或限速时会自动回退成定时快进。快进的两个参数都可以改：每隔多久跳一次（0.25–30 秒，默认 1 秒）和每次跳多远（1–120 秒，默认 10 秒）。暂停会停止快进，继续任务会恢复。

如果已保存进度约 45 秒没有增加，提示条会变红并弹一次提醒（同一轮只弹一次），里面会写明网页当前播放位置、是否在缺口附近、下一处缺口大约在哪里。智能补全会高亮正在补的那一段，播放器跑偏超过约 20 秒会提示，一段补完会告诉你下一处缺口的时间。

小于 188 字节（一个 MPEG-TS 包）的分片一律不算已保存。随后 Web Keeper 会读取前后两个已保存分片的时间戳（各取约 512 KB，不解码、不用 ffmpeg）来判断：前后本来就接得上，说明这一片没有有效媒体，标为**可跳过**，既不再重试也不参与合并；空档接近播放列表里该片的时长，说明真的缺内容，继续当缺片；判不准的一律仍按缺片处理。合并时会跳过已确认可跳过的分片，并在完成提示里说明跳过了几个。

同一套时间戳检查也用来发现任务中途的时间轴漂移 —— 长时间暂停后片源被重新编码或换了签名会出现这种情况。Web Keeper 会在接缝处记一个断点、作废过期的可跳过结论、保留已下载的分片并继续下载，合并时再把两段重排进同一条时间线。接缝处仍可能有轻微跳变，这不是 ffmpeg 级别的精确对齐。

旧的 `data/captures` 可以用下载中心的「导入旧捕获目录」接进扩展。选 captures 根目录、某个作品夹或单个清晰度夹都可以。一个作品夹下往往有多个清晰度，导入前会先列出每档的分片数和体积，默认只勾选最大的那档，确认后才开始 —— 不会自作主张把每个清晰度都导一遍。

默认是**就地引用**：分片记录直接指向你原目录里的文件，不复制、不额外占空间，秒级完成；读取和合并时才按需解密。代价是**生成视频前不能移动或删除原目录**。如果需要脱离原目录，勾选「复制一份到扩展存储」（会按原样多占一份空间）。导入后缺的部分照常用辅助抓取和智能补全补齐。

Capture 仍然是面向流媒体的兜底，不是任意网络响应录制器。多 Period DASH 和复杂独立音轨仍需后续实测与适配。
...
开发版 ZIP 位于 `dist/Web-Keeper-0.4.6.zip`。解压后，在 Edge/Chrome 的扩展管理页开启开发人员模式并选择“加载解压缩的扩展”。ZIP 不包含实验性 Helper，也不需要本地 Python 服务。默认通过浏览器下载 API 保存到正常的 Downloads；只有选择“自选文件夹”时才会遇到 Chromium 对系统文件夹根目录的授权限制。
