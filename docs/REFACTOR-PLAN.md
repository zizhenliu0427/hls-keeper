...
> 状态：0.3.7 纯扩展主流程已实现；本文同时保留后续能力边界  
> 更新日期：2026-08-03  
...
`0.2.0` 是早期技术预览。当前 `0.3.7` 已完成 popup 确认、作品聚合下载中心、设置与中英文 i18n、直链 Range 续传、HLS/DASH/CMAF、常见分离音轨、字幕、缺片时间轴、智能补全、独立删除语义和本地发布包，并修复内部页面打开、popup 横向溢出、任务状态/速度误导、InPrivate 独立运行、本次忽略语义、音频清单覆盖视频主清单、未校验坏成品和 Windows 将未知 MP4 时长显示为 13 小时的问题。默认下载先写入扩展私有 OPFS 做断点与校验，再经浏览器 Downloads API 交付成品；自选文件夹仍作为高级选项。直接下载保留并排序完整 HTTP 媒体、HLS 与 DASH 候选；MPEG-TS 使用浏览器内组件转封装为 MP4，输出验证含视频轨后才能完成和清理。Capture 仍是用户可选的 HLS 兜底路径，并增加近期媒体请求元数据队列和无扩展名分片识别；它仍会重新请求分片而非截取浏览器响应体。Helper 继续停用；DRM、复杂转码、非 CMAF/直播型 HLS 独立音轨、跨 Period/非标准 DASH 与真实目标站点适配仍受本文能力闸门约束。
...
当前实现边界：Capture 以普通 HLS 为主，观察到请求后在任务页重新获取分片，并不读取浏览器已接收的响应体。background 仅保留最近请求元数据用于任务页重开补处理；一次性 URL、严格 Referer/Origin、DASH、复杂独立音轨与直播滚动窗口仍需后续能力闸门和真实站点验证。
